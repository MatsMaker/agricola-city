...
class CityEntity {
	...
	protected fillTerrainData(): void {
		...
		this.forEachPoints((x, y) => {
			if (!this.terrain[y]) { this.terrain[y] = []}
			if (!this.terrain[y][x]) { this.terrain[y][x] = undefined}
		})
		this.forEachPoints((x, y) => {
			const randomLandTexture = grasTexturesKeys[_.random(0, grasTexturesKeys.length - 1)];				
			this.terrain[y][x] = new CityLand(x, y, grasTextures[randomLandTexture], MAP_OBJECT_TYPE.LAND);
		})
	}
	...
	protected fillObjectsData(): void {
		...
		this.forEachPoints((x, y) => {
			if (!this.objects[y]) { this.objects[y] = []}
			if (!this.objects[y][x]) { this.objects[y][x] = undefined}
		})
		this.forEachPoints((x, y) => {
				const pointData = cityData.find(d => (d.x === x && d.y === y)); // TODO need extend for set big size objects
				if (pointData) {
					const build = new CityBuild(x, y, this.getTextureByObjectType(pointData.type), pointData.type);
